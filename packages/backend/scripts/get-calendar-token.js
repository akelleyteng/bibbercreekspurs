#!/usr/bin/env node
/**
 * One-time script to get an OAuth2 refresh token for Google Calendar writes.
 *
 * Prerequisites:
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    console.log(`GOOGLE_CALENDAR_OWNER_REFRESH_TOKEN=${tokens.refresh_token}`);
    console.log('\nStore in Secret Manager for production:');
    console.log(`echo -n '${tokens.refresh_token}' | gcloud secrets versions add google-calendar-refresh-token --data-file=-`);
